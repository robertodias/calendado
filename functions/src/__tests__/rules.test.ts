...
const mockPlatformAdminToken = {
  platformAdmin: true,
  roles: {}
};

const mockOrgOwnerToken = {
  platformAdmin: false,
  roles: {
    'org-123': 'owner'
  }
...
describe('Firestore Security Rules', () => {
  ...
  describe('Platform Admin Access', () => {
    test('platform admin can read all users', async () => {
      const db = testEnv.authenticatedContext('platform-admin-123', mockPlatformAdminToken).firestore();
  ...
  describe('Organization Access Control', () => {
    ...
    test('org owner can read and write organization', async () => {
      const db = testEnv.authenticatedContext('org-owner-123', mockOrgOwnerToken).firestore();
    ...
    test('org admin can read and write organization', async () => {
      const db = testEnv.authenticatedContext('org-admin-123', mockOrgAdminToken).firestore();
    ...
    test('store manager cannot write organization', async () => {
      const db = testEnv.authenticatedContext('store-manager-123', mockStoreManagerToken).firestore();
    ...
    test('professional cannot write organization', async () => {
      const db = testEnv.authenticatedContext('professional-123', mockProfessionalToken).firestore();
    ...
    test('viewer can only read organization', async () => {
      const db = testEnv.authenticatedContext('viewer-123', mockViewerToken).firestore();
