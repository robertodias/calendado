...
export const CalendarPlaceholder: React.FC<CalendarPlaceholderProps> = ({
  ...
  const generateMockSlots = (date: Date): CalendarSlot[] => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const availableSlots = timeSlots.slice(0, Math.floor(Math.random() * 3) + 3);
  ...
  const availableDates = [...new Set(slots.filter(slot => slot.available).map(slot => slot.date))];
    ...
    <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
      ...
      <div className="space-y-4">
        {availableDates.length === 0 ? (
          <p className="text-gray-500 text-center py-8">No available dates this month</p>
        ) : (
