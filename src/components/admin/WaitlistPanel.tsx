...
const WaitlistPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== 'all') {
      q = query(waitlistRef, where('status', '==', statusFilter), orderBy('createdAt', 'desc'));
    }
  ...
  const exportToCSV = () => {
    ...
    try {
      const csvContent = [
        // Header row
        ['Email', 'Name', 'Status', 'Created At', 'Locale', 'Confirmation Sent'].join(','),
        // Data rows
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-green-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-green-800">{statusCounts.active || 0}</div>
          <div className="text-sm text-green-600">Active</div>
        </div>
        <div className="bg-blue-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-blue-800">{statusCounts.invited || 0}</div>
