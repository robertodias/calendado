...
export const AvailabilityStep: React.FC = () => {
  ...
  const loadAvailability = useCallback(async () => {
    try {
      ...
      const dates = [
        ...new Set(
          updatedSlots.filter(slot => slot.available).map(slot => slot.date)
        ),
      ].sort();
      ...
      if (preselectedDate && preselectedTime) {
        const preselectedSlot = updatedSlots.find(
          slot =>
            slot.date === preselectedDate &&
            slot.time === preselectedTime &&
            slot.available
        );
      ...
      {availableDates.length === 0 ? (
        <div className='text-center py-8'>
          ...
          <p className='text-sm text-gray-400'>
            Please try a different service or contact us directly.
          </p>
        </div>
      ) : (
              ...
              <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3'>
                {getTimeSlotsForDate(selectedDate).map(slot => (
                  <Button
                    key={slot.id}
                    variant={
                      state.selectedSlot?.id === slot.id ? 'primary' : 'outline'
                    }
        ...
        <div className='mt-6 p-4 bg-green-50 border border-green-200 rounded-lg'>
          <div className='flex items-center space-x-2'>
            ...
            <p className='text-green-800'>
              <strong>Selected:</strong> {formatDate(state.selectedSlot.date)}{' '}
              at {formatTime(state.selectedSlot.time)}
            </p>
          </div>
        </div>
      )}
