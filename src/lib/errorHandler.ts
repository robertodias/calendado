...
export const createAuthError = (message: string, code: ErrorCode = ErrorCode.UNAUTHORIZED): AppError =>
  new AppError({
    code,
    message,
    retryable: false
...
export const createBusinessError = (message: string, code: ErrorCode, retryable = false): AppError =>
  new AppError({
    code,
    message,
    retryable
...
export const createSystemError = (message: string, originalError?: Error): AppError =>
  new AppError({
...
export function shouldShowErrorToUser(error: unknown): boolean {
  if (error instanceof AppError) {
    ...
    return error.code !== ErrorCode.UNKNOWN_ERROR && 
