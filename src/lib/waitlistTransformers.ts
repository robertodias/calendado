...
import { transformDocuments, extractString, extractNullableString, extractObject, extractDate, extractNullableDate, extractBoolean } from './firestoreUtils';
 ...
 */
export function transformWaitlistEntry(data: DocumentData, docId: string): WaitlistEntry {
 ...
 */
export function transformWaitlistEntries(docs: QueryDocumentSnapshot<DocumentData>[]): WaitlistEntry[] {
  return transformDocuments(docs, transformWaitlistEntry, {
    logErrors: true,
    onError: (error, docId, field) => {
      console.error(`Failed to transform waitlist entry ${docId} field ${field}:`, error.message);
    }
  });
}
...
export function validateWaitlistEntry(entry: WaitlistEntry): boolean {
  return (
    ...
    ['pending', 'confirmed', 'invited', 'blocked', 'rejected', 'active'].includes(entry.status) &&
 ...
 */
export function filterValidWaitlistEntries(entries: WaitlistEntry[]): WaitlistEntry[] {
